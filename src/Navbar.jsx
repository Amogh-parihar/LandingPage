...
const Navbar = () => {
  ...
  // ✅ Load saved theme from localStorage when Navbar mounts
  useEffect(() => {
    ...
    const checkUserData = () => {
      ...
      if (loggedInStatus) {
        try {
          ...
          if (userDataString) {
            // Handle "undefined" string from localStorage gracefully
            const safeUserDataString =
              userDataString === "undefined" ? null : userDataString;
            const userData = safeUserDataString
              ? JSON.parse(safeUserDataString)
              : null;
  ...
  return (
    <nav className="sticky top-0 z-50 w-full flex justify-between items-center px-6 py-4 bg-white dark:bg-neutral-900 text-black dark:text-white shadow-md">
      ...
      <div className="hidden md:flex items-center space-x-6 font-semibold text-xs lg:text-lg">
        ...
        <Link to="/DiscussionPage" className="hover:text-orange-500 transition">
          Discuss
        </Link>
            ...
            {notificationOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded shadow-md z-50">
                ...
                <div className="max-h-64 overflow-y-auto">
                  <div className="px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-800">
