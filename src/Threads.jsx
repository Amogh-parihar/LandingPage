// src/components/Threads.jsx

import React, { useEffect, useRef } from "react";
import { Renderer, Program, Mesh, Triangle, Color } from "ogl";

// Vertex shader waisa hi rahega
const vertexShader = `
...
// Fragment shader mein mouse effect ko badhaya gaya hai
const fragmentShader = `
  ...
  float lineFn(vec2 st, float width, float perc, float offset, vec2 mouse, float time, float amplitude, float distance) {
    ...
    // Mouse Y-axis ka effect badhaya (0.2 se 1.5)
    float finalAmplitude = amplitude_normal * amplitude_strength * amplitude * (1.0 + (mouse.y - 0.5) * 1.5);
    // Mouse X-axis ka effect badhaya (1.0 se 2.5)
    float time_scaled = time / 10.0 + (mouse.x - 0.5) * 2.5;
    // --- END OF UPDATE ---
...
const Threads = ({
  ...
  useEffect(() => {
    ...
    function update(t) {
      ...
      // Animation ki speed badhayi (0.0005 se 0.0015)
      program.uniforms.iTime.value = t * 0.0015;
      // --- END OF UPDATE ---
